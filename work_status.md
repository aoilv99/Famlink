...
## 今おかしいところ
1.新規登録とログインがうまくいかないときがある→サーバー再起動してlocalhostも再起動しなおしたら新規登録できる
2.最近の家族の様子がうまく家族間で表示されない。
    ・アカウントごとに名前の横のまるの色を変えたいのに変わらない
    ・リアルタイムで家族の感情が表示できない（リロードして取得でもいいけど、リロードしたら家族に参加してない状態になるみたいなわけわからんことになる）
3.家族の招待がうまくいかないときがある
    ・特に参加はできるけど招待した側が入れない（サーバーエラー500みたいなのが出る⇒これは新規登録がうまくいかないときも同じの出る）
    　Failed to load resource: the server responded with a status of 500 (Internal Server Error)

## 未実装
1.会いたいボタンの処理
    ・日時とか決めて送信ボタンを押したときに通知画面に内容が通知されるようにしたい
    ・その通知からボタンを押すと、日時の選択ができてそれも通知する→家族全員の予定が会う日に合うのがっけていする→通知
2.名前変更
